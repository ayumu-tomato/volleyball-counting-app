...
# 1. 設定 & JS制御
# ==========================================
st.set_page_config(page_title="Volleyball Scouter Ver.4.6", layout="wide")
...
def update_score(winner):
    ...
    if winner == 'my':
        ...
        # サイドアウト (R -> S) の時だけローテを回す
        if current_phase == 'R':
            rotate_team()
            st.toast("Sideout! Rotated 🔄", icon="⭕")
        else:
            # ブレイク (S -> S) は回さない
            st.toast("Break Point! (No Rot) ⏹️", icon="⭕")
...
def commit_record(quality, winner=None):
    ...
    if winner:
        ...
        if (skill in ['A','B','S'] and quality=='#') or (skill=='A' and quality=='T'):
            ...
            st.toast("Saved. (No Score Change)", icon="✅")
